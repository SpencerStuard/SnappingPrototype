...
half4 fragForwardBaseInternal_TCP2(VertexOutputForwardBase i)
{
	...
	half4 c = TCP2_BRDF_PBS(s.diffColor, s.specColor, s.oneMinusReflectivity, SMOOTHNESS, s.normalWorld, -s.eyeVec, gi.light, gi.indirect,
...
half4 fragForwardAddInternal_TCP2(VertexOutputForwardAdd i)
{
	...
	half4 c = TCP2_BRDF_PBS(s.diffColor, s.specColor, s.oneMinusReflectivity, SMOOTHNESS, s.normalWorld, -s.eyeVec, light, noIndirect,
...
/*
struct VertexOutputDeferred_TCP2
{
...
VertexOutputDeferred_TCP2 vertDeferred_TCP2 (VertexInput v)
{
	VertexOutputDeferred_TCP2 o;
	UNITY_INITIALIZE_OUTPUT(VertexOutputDeferred_TCP2, o);
...
//Original Unity structure:
// RT0: diffuse color (rgb), occlusion (a)
// RT1: spec color (rgb), smoothness (a)
// RT2: normal (rgb), --unused, very low precision-- (a)
// RT3: emission (rgb), --unused-- (a)

//TCP2 Structure:
// RT0: diffuse color (rgb), ramp smoothing (a)
// RT1: spec color (rgb), smoothness (a)
// RT2: normal (rgb), --unused, very low precision-- (a)
// RT3: emission (rgb), --unused-- (a)

void fragDeferred_TCP2 (
VertexOutputDeferred_TCP2 i,
...
)
{
	...
	//pack smoothness & threshold into rt0.a
	// - convert to 4bit precision [0:15]
	// - shift and add values
	// - clamp to avoid error when _RampThreshold == 1
	float tcp2_smooth_threshold = ((floor(_RampSmooth*15.0) * 15.0) + floor(clamp(_RampThreshold, 0, 0.99)*15.0)) / 255.0;

	//outDiffuse = half4(s.diffColor, occlusion);
	outDiffuse = half4(s.diffColor, tcp2_smooth_threshold);
